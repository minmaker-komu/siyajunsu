<?php
    ...
    $sql = "INSERT INTO tb_notice(title, content, writer_name, writer_id, date, hit, file) VALUES ('$title', '$content', '$writer_name', '$writer_id', now(), 0, '$arrayString');";

    $res = mysqli_query($conn, $sql);

    if($res) {
        echo "<script>alert('공지사항이 작성되었습니다.');";
        echo "window.location.replace('notice_list.php');</script>";
    } else {
