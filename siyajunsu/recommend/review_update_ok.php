<?php
    ...
    $sql_update = "UPDATE tb_review SET title = '$title', content = '$content', place = '$place', concert_date = '$date' WHERE idx=$id;";

    if(!$filename){
        $sql_update = "UPDATE tb_review SET title = '$title', content = '$content', place = '$place', concert_date = '$date' WHERE idx=$id;";
    } else {
        $sql_update = "UPDATE tb_review SET title = '$title', content = '$content', place = '$place', concert_date = '$date',  file = '$arrayString' WHERE idx=$id;";
    }
        
    $res_update = mysqli_query($conn, $sql_update);

    if($res_update) {
        echo "<script>alert('공연후기가 수정되었습니다.');";
        echo "window.location.replace('review_view.php?idx=$id');</script>";
    } else {
